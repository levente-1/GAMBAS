...
class Unet3D(nn.Module):
    def __init__(
        ...
        def temporal_attn(dim): return EinopsToAndFrom('b c f h w', 'b (h w) f c', Attention(dim, heads=attn_heads, dim_head=attn_dim_head, rotary_emb=rotary_emb))
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.Tanh() # according to the paper, there should be an additional tanh here
        )
    ...
    def forward(
        ...
        time = torch.mean(time, dim=(1,2,3,4)) # mean(W)
...
class WBlock(nn.Module):
    def __init__(self):
        ...
        sequence = [nn.Upsample((16, 16, 16), mode='trilinear', align_corners=False),
                    Block(1, 32),
                    nn.Upsample((32, 32, 32), mode='trilinear', align_corners=False),
                    Block(32, 64),
                    nn.Upsample((64, 64, 64), mode='trilinear', align_corners=False),
                    Block(64, 32),
                    nn.Conv3d(32, 1, 1),
                    nn.Tanh()]
...
# Likely everything below this line is not needed
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, cond=None, cond_scale=1., batch_size=16):
        ...
        _sample = self.p_sample_loop((batch_size, channels, num_frames, image_size, image_size), cond=cond, cond_scale=cond_scale)
